...
#define UART_RX_PAL_MODE 1 // Common default alternate function index for WB32 UART RX
#endif
...
void lpwr_exti_init(void) {
    ...
    for (uint8_t i = 0; i < ARRAY_SIZE(col_pins); i++) {
        if (col_pins[i] != NO_PIN) {
            palSetLineMode(col_pins[i], PAL_MODE_OUTPUT_OPENDRAIN);
...
#ifndef LPWR_UART_WAKEUP_DISABLE
    palSetLineMode(UART_RX_PIN, PAL_MODE_INPUT);
    waitInputPinDelay();
